...
class WorkflowStepper(ctk.CTkFrame):
    ...
    def __init__(self, master, palette: dict):
        super().__init__(
            ...
            border_width=1,
            border_color=palette.get("glass_border", palette["border"]),
        )
        self.palette = palette
        self.labels = []
        divider_color = palette.get("divider", palette.get("border", "#E4EAF3"))
        for i, step in enumerate(self.STEPS):
            lbl = ctk.CTkLabel(self, text=f"○ {step}", font=ctk.CTkFont(family="Segoe UI", size=13, weight="bold"))
            lbl.grid(row=0, column=i * 2, padx=(14, 4), pady=12, sticky="w")
            self.labels.append(lbl)
            if i < len(self.STEPS) - 1:
                ctk.CTkLabel(self, text="━━━━", text_color=divider_color).grid(row=0, column=i * 2 + 1, sticky="ew", padx=4)
