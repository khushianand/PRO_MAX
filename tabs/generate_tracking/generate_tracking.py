...
class GenerateTrackingTab(ctk.CTkFrame):
    ...
    def _validate_form(self):
        required_ok = bool(self.raw_file.get().strip() and self.raw_sheet.get().strip() and self.output_file.get().strip())
        if self.master_file.get().strip():
            required_ok = required_ok and bool(self.master_sheet.get().strip())
        self.run_btn.configure(state="normal" if required_ok else "disabled")
        self.form_status.configure(
            text="Ready to run" if required_ok else "Fill required fields to enable Run",
    ...
    def run(self):
        ...
        try:
            ...
            if (
                ...
                output_new_df = build_3uk_qualys_template_sheet_df(new_df)
                output_old_df = build_3uk_qualys_template_sheet_df(old_df)
                unique_df = build_3uk_qualys_unique_sheet_df(raw_df)
            
            else:
            
                output_new_df = new_df
                output_old_df = old_df
